...
with h5py.File(maskpath) as f, h5py.File(catpath) as g:
    Mask = np.array(f['baseline_mcal_mask_2m'])
    length = Mask.sum()
    flux_r, flux_i, flux_z = np.array(g['mcal_flux_2m_dered_sfd98']).T[:,Mask]
    flux_err_r, flux_err_i, flux_err_z = np.array(g['mcal_flux_err_2m_dered_sfd98']).T[:,Mask]
    coadd_object_id = np.array(g['id'])[Mask]
    ra, dec = np.array(g['RA'])[Mask], np.array(g['DEC'])[Mask]
    colnames = [f'mcal_flux_2m_dered_sfd98_{band}' for band in 'riz']+\
               [f'mcal_flux_err_2m_dered_sfd98_{band}' for band in 'riz']+\
               ['id', 'RA', 'DEC']
    ...
    for i, df_ in enumerate(dfs):
        df_.to_hdf(f'%s/classify_sfd98_2m/cat_{i:03}.hdf5'%outpath, key='df')